...
public class gifting_mechanism {
     public void gift() throws IOException{
        ...
        for(p=0;p<no_of_boys;p++){
         ...
         File fp=new File("log4.txt");
	         Boolean check;
		check=fp.createNewFile();
        try (FileWriter fw = new FileWriter("log4.txt"); BufferedWriter bw = new BufferedWriter(fw)) {
        ...
        for ( i = 0; i < no_of_girls-1; i++){
            ...
            for(q=0;q<no_of_boys;q++){
                if(x[q].get_status()==true&&x[q].gfname == y[p].get_name()){
                        if(u>0){
