        ...
        while(n>0){
            ...
            for(q=0;q<no_of_girls;q++){
                if((x[p].girl_meets_eligibility(y[q].get_maintenance_budget(),y[q].get_attractiveness()))&&(y[q].boy_meets_eligibility(x[p].budget))){
                    if (x[p].get_status()==false && y[q].get_status()==false) {
                        x[p].gfname = y[q].get_name();
                        y[q].bfname = x[p].get_name();
                        x[p].set_status(true);
                        y[q].set_status(true);
         ...
         File fp=new File("log5.txt");
	         Boolean check;
		check=fp.createNewFile();
        try (FileWriter fw = new FileWriter("log5.txt"); BufferedWriter bw = new BufferedWriter(fw)) {
